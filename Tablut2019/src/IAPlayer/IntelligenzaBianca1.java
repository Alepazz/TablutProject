...
public class IntelligenzaBianca1 implements IA {
	...
	/**
	 * Aggiunge un nuovo stato all'elenco degli stati che sono stati visitati fin'ora, nell'indice corrispondente
	 * @param s StateTablut ovvero lo stato che si vuole aggiungere alla lista
	 * @param indice Indice in cui inserire l'elemento
	 */
	public void setState(StateTablut s, int indice) {
		listState.put(indice, s);
	}
	...
	public boolean checkDraw(StateTablut s) {
		if(listState.isEmpty()) {
			...
			if(common.getNumberPawns(s) != common.getNumberPawns(listState.get(listState.size()-1))) {
				listState.clear();
				listState.put(listState.size(), s); //TODO: CONTROLLO
				return false;
	...
	private class HeuristicValuator implements Runnable {
		private IntelligenzaBianca1 ia;
		private boolean isRunning;
		private int timeToStopHeuristicValuator;
		
		public HeuristicValuator(IntelligenzaBianca1 ia, int timeToStopHeuristicValuator){
			this.ia = ia;
			this.isRunning=true;
	...
	private class TreeGenerator implements Runnable {
		...
		public IdentityHashMap<Integer, Nodo> mossePossibiliComplete(Nodo node) throws IOException, BoardException, ActionException, StopException, PawnException, DiagonalException, ClimbingException, ThroneException, OccupitedException, ClimbingCitadelException, CitadelException{
			...
			for(int i=0; i<righeDaControllare && isRunning; i++)
			{
				...
				for(int j=0; j<colonneDaControllare && isRunning; j++)
				{
					...
					if(node.getTurn().equalsTurn(State.Turn.BLACK.toString()) && State.Pawn.BLACK.equalsPawn(node.getBoard()[i][j].toString()) && isRunning)
					{
						if(statiSimm && j==4)
						{
							for(int nod=0; nod<mossePossibiliPedinaCS(node, i, j).size(); nod++)
							{
						...
						else
						{
							if(simmV && simmO && j==4)
							{
								for(int nod=0; nod<mossePossibiliPedinaCCS(node, i, j).size(); nod++)
								{
									listaMossePossibili.put(indice, mossePossibiliPedina(node, i, j).get(nod));
									indice++;
									//System.out.println("Grandezza mosse possibili: " + listaMossePossibili.size());
							...
							else
							{
								if(simmV && j==4)
								{
									for(int nod=0; nod<mossePossibiliPedinaSV(node, i, j).size(); nod++)
									{
										listaMossePossibili.put(indice, mossePossibiliPedina(node, i, j).get(nod));
										indice++;
										//System.out.println("Grandezza mosse possibili: " + listaMossePossibili.size());
								...
								else
								{
									if(simmO && i==4)
									{
										for(int nod=0; nod<mossePossibiliPedinaSO(node, i, j).size(); nod++)
										{
											listaMossePossibili.put(indice, mossePossibiliPedina(node, i, j).get(nod));
											indice++;
											//System.out.println("Grandezza mosse possibili: " + listaMossePossibili.size());
									...
									{
										for(int nod=0; nod<mossePossibiliPedina(node, i, j).size(); nod++)
										{
											listaMossePossibili.put(indice, mossePossibiliPedina(node, i, j).get(nod));
											indice++;
											//System.out.println("Grandezza mosse possibili: " + listaMossePossibili.size());
			...
			for(int i=0; i<listaMossePossibili.size(); i++)
			{
				listaMossePossibili.get(i).setPadre(node);
			}
		...
		private IdentityHashMap<Integer, Nodo> mossePossibiliPedina(Nodo node, int riga, int colonna) throws IOException, BoardException, ActionException, StopException, PawnException, DiagonalException, ClimbingException, ThroneException, OccupitedException, ClimbingCitadelException, CitadelException
		{
			...
			{
				for(int nod=0; nod<mossePossibiliPedinaSopra(node, riga, colonna).size(); nod++)
				{
			...
			if(canMoveDown(node.getStato(), riga, colonna) && isRunning)
			{
				...
				for(int nod=0; nod<mossePossibiliPedinaSotto(node, riga, colonna).size(); nod++)
				{
			...
			{
				for(int nod=0; nod<mossePossibiliPedinaSinistra(node, riga, colonna).size(); nod++)
				{
			...
			{
				for(int nod=0; nod<mossePossibiliPedinaDestra(node, riga, colonna).size(); nod++)
				{
